...
Node.prototype.isValid = function () {
    ...
    if (!this.every(isValidCityIndex, this))
        return false;
    return true;
};
...
process.stdin.on('end', function () {
    ...
    for (var i = 1; i < lines.length; i++) {
        ar = lines[i].split(' ');
        cities[i] = new City(parseInt(ar[0], 10), parseInt(ar[1], 10), parseInt(ar[2], 10));
    }
    ...
    while (generation < NUM_GENERATION) {
        ...
        if (bestStayingCount == Math.floor(NUM_GENERATION / 5) || generation == NUM_GENERATION)
            break;
